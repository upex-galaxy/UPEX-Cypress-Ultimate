...
import { BoardMembersPage } from '@api/Trello/GX3-5811-boardMembers.Api';
import { type UserData, type ApiResponse } from '@api/Trello/types/GX3-5811-boardMembers.Api.Types';
...
describe('GX3-5811 | Trello (API) | Members | API Endpoint: Get the Members of a Board', () => {
	...
	before('PRC: El usuario tiene que tener boards con miembros y estar conectado.', function () {
		cy.fixture('data/Trello/GX3-5811-boardMembers')
			...
			.then(() => {
				...
				if (!token || !key) {
					throw new Error('Las variables de entorno TRELLO_KEY o TRELLO_TOKEN no están definidas.');
				}
				...
				authTrello.authenticate(requestData).then((header: string) => {
					...
					boardMembersPage.getUserId(authHeader, urlGetMemberId).then((idMember: string) => {
						fixtureData.data.idMember = idMember;
					});

					const urlPostBoard: string = authTrello.buildUrl(fixtureData.url.post.board, {
						protocol: fixtureData.url.protocol,
						host: fixtureData.url.host,
						basePath: fixtureData.url.basePath,
					...
					boardMembersPage.createBoard(authHeader, urlPostBoard).then(idBoard => {
						...
						const urlPutMemberToBoard: string = authTrello.buildUrl(fixtureData.url.put.memberToBoard, {
							protocol: fixtureData.url.protocol,
							host: fixtureData.url.host,
							basePath: fixtureData.url.basePath,
							idBoard: fixtureData.data.idBoard,
							idMember: fixtureData.data.idMember,
							endPath: fixtureData.url.endPath,
							myKey: fixtureData.auth.key,
							myToken: fixtureData.auth.token
						});
	...
	after('PSC: Delete created board.', () => {
		const urlDeleteBoard: string = authTrello.buildUrl(fixtureData.url.delete.board, {
			protocol: fixtureData.url.protocol,
			host: fixtureData.url.host,
			basePath: fixtureData.url.basePath,
			idBoard: fixtureData.data.idBoard,
